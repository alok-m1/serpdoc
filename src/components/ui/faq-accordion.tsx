...
import { CheckIcon, ChevronDownIcon, Link2Icon } from "lucide-react"
...
const FaqAccordionContext = createContext<{ openItem: (value: string) => void }>({
  openItem: () => {},
})
  ...
  return (
    <AccordionPrimitive.Header className="flex items-center">
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          "group/faq-trigger flex w-full min-w-0 flex-1 cursor-pointer items-center gap-3 rounded-md px-2 py-3 text-left outline-none",
      ...
      >
        <span
          className={cn(
            "hidden shrink-0 text-xs font-medium tabular-nums text-muted-foreground/70 select-none sm:block",
        ...
        <ChevronDownIcon
          className={cn(
            ...
            "group-aria-expanded/faq-trigger:rotate-180 group-aria-expanded/faq-trigger:text-primary"
          )}
        />
      </AccordionPrimitive.Trigger>
      ...
      <button
        ...
        className={cn(
          "mr-1 flex size-7 shrink-0 items-center justify-center rounded-md text-muted-foreground/60 transition-colors duration-150",
...
function FaqContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "h-(--accordion-panel-height) px-2 pt-0.5 pb-6 pr-4 data-ending-style:h-0 data-starting-style:h-0 sm:pl-9",
          className
        )}
      >
        <div
